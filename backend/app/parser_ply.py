...
class ConvertidorParser:
    ...
    def __init__(self):
        self.lexer_obj = ConvertidorLexer()
    ...
    def p_error(self, p):
        if p:
            error_msg = f"Error de sintaxis en línea {p.lineno}, token '{p.value}' ({p.type})"
    ...
    def parse(self, texto: str) -> tuple:
        """
        Parsea el texto de entrada.
        
        Returns:
            tuple: (conversiones, errores)
        """
        self.conversiones = []
        self.errores = []
        
        # Primero tokenizar con el lexer
        self.lexer_obj.reset()
        
        # Parsear
        self.parser.parse(texto, lexer=self.lexer)
